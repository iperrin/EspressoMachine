...
// Coffee Parameters
#define tempSetPoint 98
#define pressSetPoint 9
#define preInfusionTime 0
#define preInfusionSpeed 1800
#define switchRatio 0.75
...
void autoSequence() {
  ...
  while (!ts.touched()) {
    ...
    //check conditions to perform a one-time reset of the integrator error
    if (integralReset && pressure > pressSetPoint*switchRatio) {
      //offset = PID_Output;  
      myPID.reset();
      PID_Output = 0;
      integralReset = false;
    ...
    myPID.run();
    double voltSet = (PID_Output) * 4096;
    //voltSet = voltSet+offset;
    dac.setVoltage(voltSet, false);
    Serial.println(PID_Output+offset);
